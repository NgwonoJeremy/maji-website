## TEAM 4 CHARTER
____
### Team Members and roles
- Marie-Ange - API Lead
- John Mwangi - DevOps/Docs Lead
- David Mukoya - Intergration/QA Lead
- Jeremy Raphael - Backend Dev
____
### App Summary

A Kenyan 🇰🇪 frontend webapp that seeks to minimize struggle of sourcing reliable water solutions, built to connect the customers to the vendors.
Current solutions,have a high minimum quantity for each order.This does not match the user's situation.
MAJI-PLATFORM however we provide access to water from any quantity,from **20L** which is the minimum functional capacity.
___

### APP Audit
#### Customer Dashboard
- Username
- User email
- User Phone number
- Location
- Quantity
- Delivery time
- Payment
- Payment method – Cash or M-Pesa Amount – in Ksh
#### Vendor Dashboard
- Feedback Vendor
- Business Entity/ Station Name
- Contact Representative Full name
- M-Pesa Connected Phone Number
- Primary Target Estate
- Daily Capacity Admin
#### Admin Dashboard
- Total Orders
- Vendor Verification
- Total Revenue
____
### Ring Position

RING POSITION

[ Team 3 ] -- Consume API --> **[ Team 4 ]** -- Produce API --> [ Team 5 ]

