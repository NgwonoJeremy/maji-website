...
| **GET** | `/customers/paymentmethod` | Returns a list of the customers' payment method | SkillBridge needs to read Maji customers' M-Pesa payment history (payment method only, not amount). |
| **DELETE** | `/customer/{id}` | Deletes a customer's record | SkillBridge may need to delete a customer's record when he/she has been inactive for too long. |
| **GET** | `/vendors/deliverytime` | Returns a list of the vendor's delivery times. | SkillBridge needs to read delivery time windows (e.g., morning/afternoon) per estate, to schedule skill session reminders during waiting periods when customers are likely free. |
